...
public class TabReaderV5{
	...
	private Pattern tabPatF = Pattern.compile(tabRegexFL);
	private Pattern tabPatS = Pattern.compile(tabRegexSL);
...
//	
	 ...
	 */
	public TabReaderV5(File file, int string_count, int measure) throws Exception { //basic, parameter-driven constructor
		this.string_count = string_count;
		this.file = file;
		this.eof = false;
		// measures start at 1, index 0 exists - but is reserved for tuning data
	...
	public void readMeasure() throws Exception {
		...
		if(curr_measure >= tabLine[0].length) {
			...
			curr_measure ++;
			if(this.measure[0].length() == 0) {
				readMeasure();
				System.out.println("DEBUG: REPEAT?!?!?!");
			}
		}
	}
	...
	private void evaluateLine() throws Exception {
		...
		try {
			...
			while(sc.hasNext()) {
				...
				temp = sc.nextLine();
				temp = temp.trim();
				Matcher tabMat = tabPatF.matcher(temp);
				if(tabMat.matches()) {
					...
					throw new LineErrorException("Bad formatting (First Line): detected at line " + this.scanLine, this.scanLine, temp);
				}
			}
			...
			for(int i=1; i<this.string_count; i++) {
				...
				Matcher tabMat = tabPatS.matcher(temp);
				if(tabMat.matches()) {
					uTab[i] = temp;
				}
				else {
					throw new LineErrorException("Bad formatting: (subsequent lines) detected at line " + (this.scanLine + i), this.scanLine + i, temp);
				}
			}
			...
			for(int i=0; i<bTab[0].length; i++) {
				...
				for(int j=1; j<bTab.length; j++) {
					if(measureLength != bTab[j][i].length()) {
						...
						//check if exposed digit is number -> repeat -> cull and submit?
						
						if(measureLength > bTab[1][i].length() && notAdjusted) {
							Matcher nOMat = nOPat.matcher(bTab[0][i].substring(bTab[j][i].length(), measureLength));
							
							System.out.println("DEBUG: diffence in line lengths " + (measureLength - bTab[j][i].length()));
							System.out.println("DEBUG: " + (bTab[0][i].substring(0,bTab[j][i].length())));
							
							if(nOMat.matches()) {
								measureLength = bTab[j][i].length();
								//bTab[0][i] = bTab[0][i].substring(0, measureLength);
								notAdjusted= false;
								// *******************************************************************************************************
							}else {
								throw new LineErrorException("Bad content: (repeat check) the length of measure " + i + " in line " + (this.scanLine + j) + " is different from the line above", this.scanLine + j, bTab[j][i]);
